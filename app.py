...
# 業展處主管代稱。
# 真實姓名屬於個人資料,不放進 repo —— 這個 repo 是公開的。
# 部署時以環境變數 DEPARTMENT_MANAGERS_JSON 提供真實對照表即可,例如:
#   {"北一業展一處": "王小明", "北一業展二處": "李小華"}
# 未設定或格式錯誤時顯示代稱,bot 仍可正常運作。
_DEFAULT_MANAGERS = {
    "北一業展一處": "主管A", "北一業展二處": "主管B", "北一業展三處": "主管C",
    "北一業展四處": "主管D", "北一業展五處": "主管E", "北一業展六處": "主管F",
    "北一業展七處": "主管G",

    "北二業展一處": "主管H", "北二業展二處": "主管I", "北二業展三處": "主管J",
    "北二業展四處": "主管K", "北二業展五處": "主管L", "北二業展六處": "主管M",
    "北二業展七處": "主管N",

    "桃竹苗業展一處": "主管O", "桃竹苗業展二處": "主管P", "桃竹苗業展三處": "主管Q",
    "桃竹苗業展四處": "主管R",

    "中區業展一處": "主管S", "中區業展二處": "主管T", "中區業展三處": "主管U",
    "中區業展四處": "主管V", "中區業展五處": "主管W", "中區業展六處": "主管X",
    "中區業展七處": "主管Y",

    "南區業展一處": "主管Z", "南區業展二處": "主管AA", "南區業展三處": "主管AB",
    "南區業展四處": "主管AC",
}


def _load_managers():
    raw = (os.environ.get("DEPARTMENT_MANAGERS_JSON") or "").strip()
    if not raw:
        return dict(_DEFAULT_MANAGERS)
    try:
        data = json.loads(raw)
        if not isinstance(data, dict):
            raise ValueError("必須是物件")
    except (ValueError, TypeError) as e:
        print(f"[warn] DEPARTMENT_MANAGERS_JSON 解析失敗({e}),改用代稱")
        return dict(_DEFAULT_MANAGERS)
    # 只覆蓋有提供的處室,其餘維持代稱
    return {**_DEFAULT_MANAGERS, **{k: str(v) for k, v in data.items()}}


DEPARTMENT_MANAGERS = _load_managers()
