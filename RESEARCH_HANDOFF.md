...
## 五之四、抓取端的三個教訓(都是這個 session 踩出來的)

### 1. 「服務拒絕」不能呈現成「查無資料」

`fetch_financials.py` 原本把 HTTP 402(FinMind 配額用盡)和「查無資料」
都回傳空陣列,於是最後 10 檔被記成「無財報,略過」—— 南電、力積電
當然有財報。**這種錯不會拋例外,只會讓人相信一個假的結論。**

而且配額用盡後又空轉四十分鐘(每檔重試四次才放棄),產出是零。

現在:`get()` 遇 402/429 拋 `QuotaExhausted`;主迴圈接到就停止本輪、
保留成果、印出明確訊息;產出用 `no_data`(有回應但查無)與
`incomplete_from`(本輪沒跑完)分開記錄。網頁也據此顯示
「查無財報」或「尚未抓取」,不再一律說「可能尚未被收錄」。

> 例外處理要涵蓋主迴圈**以外**的部分。第一版只包了主迴圈,
> 但基準判別跑在它之前,配額若在那裡用盡會變成 traceback。

### 2. FinMind 的配額是每小時制,撞到就該停,不是重試

實測:一輪約能抓 100~120 檔(每檔 3 個 dataset)就撞 402。
取消後立刻重跑會撞上同一批已耗盡的額度,比等待更慢。
續抓機制(每 10 檔落地一次、啟動時跳過已完成的)是必要的,
`fetch_pit.py` 與 `fetch_financials.py` 都有。

### 3. 資料格式的契約要寫在註解裡

`docs/data/stocks.json` 的 `name` **只存公司名**,代號由網頁自己加
(`loadFeed` 的 `${id} ${s.name}`)。存成「1101 台泥」會被再加一次前綴,
變成「1101 1101 台泥」。

這個 bug 修了兩次:第一次是名稱缺失(用代號充當,674 檔裡 554 檔顯示
「0050 0050」),補上名稱後又因為存了代號而重複。兩次都**沒有錯誤、
沒有版面異常**,只有實際去看畫面上的字才會發現。

> 驗證清單裡要有「內容對不對」,不能只有「會不會壞」。

### 抓取工具一覽

| 檔案 | 用途 | 請求數 |
|---|---|---:|
| `research/update_quotes.py` | 增量補行情,用證交所全市場單日快照 | 每個交易日 1 次 |
| `research/fill_names.py` | 補中文名稱與市場別 | 1 次 |
| `research/add_stocks.py` | 加指定個股(支援上櫃) | 每檔 1 次 |
| `research/fetch_financials.py` | 三張財報,支援續抓 | 每檔 3 次 |
| `tools/make_station_from_pit.py` | 由 PIT 日線產生檢測站資料(不需網路) | — |
