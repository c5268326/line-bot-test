...
| 原始 Block                          | 台股改寫                                                    |
|-------------------------------------|--------------------------------------------------------------|
| SentimentBlock（FG逆向指標+幣種輪動+BTC主導率） | MacroBlock：用 macro.py 的總經燈號分數當市場情緒背景           |
...
class FactorBlock:
    """對應 TechnicalBlock：用已驗證的因子組合對候選股打分排名，取代威科夫/SMC 判斷。"""
    ...
    def run(self, ctx: MarketContext, config: BacktestConfig,
        ...
        if snapshot.empty:
            ...
            return ctx
        snapshot["score"] = F.composite_score(snapshot, config.factor_weights)
...
class DecisionBlock:
    ...
    def run(self, ctx: MarketContext, config: BacktestConfig, prices: pd.DataFrame) -> MarketContext:
        ...
        top_n = config.defensive_top_n if ctx.macro_regime == "HEADWIND" else config.top_n
        ...
        for _, row in candidates.iterrows():
            ...
            if ctx.macro_regime == "TAILWIND" and row["score"] >= median_score:
                decision.regime, decision.confidence, decision.position_weight = "TREND_ALIGNED", "HIGH", 1.0
            elif ctx.macro_regime == "HEADWIND" and row["score"] > candidates["score"].quantile(0.9):
                # 總經逆風，但個股分數依然是候選中的前段班 —— 對應原系統「逆勢反轉單，
                # 信心該打折」的邏輯：允許進場，但部位權重砍半。
                decision.regime, decision.confidence, decision.position_weight = "COUNTER_TREND", "MID", 0.5
            elif ctx.macro_regime == "HEADWIND":
                decision.veto_reason = "總經逆風且個股分數不夠突出，比照原系統「方向不明確」邏輯不進場。"
                ctx.vetoed.append(decision)
                continue
            else:
...
def run_pipeline(config: BacktestConfig, prices: pd.DataFrame, fundamentals: pd.DataFrame,
    ...
    """串起五個 Block，等同原系統的主循環（這裡是單次快照，不是每小時輪詢）。"""
